...
class ScanReceiptApiProvider {
  ...
  Future<StateModel?> uploadReceipt(FormData data) async {
    try {
      final response = await ObjectFactory().apiClient.uploadReceipt(data);
      ...
      if (e.response != null && e.response!.statusCode == 500) {
        ...
        return StateModel.error(
            "Oops! It looks like the server isn't responding! Please try again later");
        // return response!;
  ...
  Future<StateModel?> showScanHistory(
      ScanReceiptHistoryRequest scanReceiptHistoryRequest) async {
    try {
      // 404
      final response = await ObjectFactory()
          .apiClient
          .showScanHistory(scanReceiptHistoryRequest);
      // print(response.toString());
      if (response.statusCode == 200) {
